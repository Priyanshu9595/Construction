export type ProjectStatus = "In Progress" | "Completed" | "On Hold" | "Delayed";
...
export const tasks = [
  { projectId: "prj-ocean", name: "Brick Work - 2nd Floor", progress: 75, status: "In Progress" },
  { projectId: "prj-ocean", name: "Plastering - Tower B", progress: 40, status: "In Progress" },
  { projectId: "prj-ocean", name: "Flooring - Tower A", progress: 20, status: "Pending" },
  { projectId: "prj-green", name: "Column Casting", progress: 100, status: "Completed" },
  { projectId: "prj-green", name: "Beam Shuttering", progress: 60, status: "In Progress" },
  { projectId: "prj-tech", name: "Foundation Audit", progress: 30, status: "Blocked" },
];
...
export const incidents = [
  { projectId: "prj-ocean", title: "Material Delay", severity: "Medium", date: "17 May 2024" },
  { projectId: "prj-ocean", title: "Drawing Approval Pending", severity: "High", date: "17 May 2024" },
  { projectId: "prj-green", title: "Labour Shortage", severity: "Low", date: "16 May 2024" },
];
...
export function getProjectReport(projectId: string) {
  return siteReports.find((report) => report.projectId === projectId) ?? siteReports[0];
}

export function getPlanSummary() {
  return companies.reduce<Record<string, { companies: number; revenue: number }>>((summary, company) => {
    const current = summary[company.plan] ?? { companies: 0, revenue: 0 };
    summary[company.plan] = {
      companies: current.companies + 1,
      revenue: current.revenue + company.subscriptionRevenue,
    };
    return summary;
  }, {});
}
