...
export default function StoreDashboard() {
  const q = useQuery({
    ...
    queryFn: () => apiGet("/api/store/dashboard"),
  });
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-lg border bg-white p-5 shadow-sm">
        <h1 className="text-2xl font-extrabold">Store Manager Dashboard</h1>
        <p className="text-sm text-slate-500">{q.data.store.name}</p>
      </div>
        ...
        {cards.map((card) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-2xl font-extrabold">{String(card.value)}</p>
            </div>
          );
        })}
      </div>
...
<table className="w-full text-sm">
          ...
          <tbody>
            {q.data.inventory.map((row) => (
              <tr key={row.materialId} className="border-b border-slate-50">
                <td className="py-3 font-bold">{row.materialName}</td>
                <td className="text-center">{row.received}</td>
                <td className="text-center">{row.issued}</td>
                <td className="text-center">{row.availableStock}</td>
                <td className="text-center font-bold">{row.status}</td>
                <td className="text-right">{formatINR(row.stockValue)}</td>
              </tr>
            ))}
          </tbody>
        </table>
