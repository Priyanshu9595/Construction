...
export default function WorkerSalarySlips() {
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      return json.salarySlips as SalarySlip[];
    }
  });

  if (isLoading) return <div className="flex justify-center p-12"><Loader2 className="animate-spin text-blue-600" size={32} /></div>;
  if (isError) return <div className="text-red-600 p-8 text-center font-bold">Failed to load salary slips.</div>;

  return (
    <div className="mx-auto max-w-5xl space-y-8 p-4 sm:p-6 pb-20">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Salary Slips</h1>
        <span className="flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-2 text-sm font-bold text-emerald-700 border border-emerald-100">
          <IndianRupee size={18} /> {data?.length || 0} Slips
        </span>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {data?.length === 0 ? (
          <div className="col-span-full rounded-3xl border border-slate-200 bg-white p-12 text-center shadow-xl shadow-slate-200/40">
            <h3 className="text-xl font-bold text-slate-700">No salary slips generated yet.</h3>
          </div>
        ) : (
          data?.map((slip) => {
            const [year, month] = slip.month.split('-');
            const parsedDate = new Date(parseInt(year), parseInt(month) - 1);
            const monthName = parsedDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
            return (
              <div key={slip._id} className="relative flex flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/40 hover:border-emerald-300 transition-all hover:-translate-y-1">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`inline-flex rounded-full px-2 py-1 text-[10px] font-black uppercase tracking-widest ${slip.paymentStatus === 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                    {slip.paymentStatus}
                  </span>
                </div>
