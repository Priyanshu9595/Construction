...
export default function SiteDashboard() {
  ...
  const dashboard = useQuery({
    ...
    queryFn: () =>
      apiGet(`/api/projects/${projectId}/sites/${siteId}/dashboard`),
  });

  if (dashboard.isLoading)
    return <StateCard text="Loading site dashboard..." />;
  ...
  const cards = [
    {
      ...
      value: data.kpis.labourToday,
      icon: Users,
      color: "text-slate-900",
    },
    {
      label: "Today's Work",
      value: `${data.kpis.workProgressToday}%`,
      icon: HardHat,
      color: "text-blue-600",
    },
    {
      label: "Materials Used",
      value: data.kpis.materialsUsed,
      icon: Package,
      color: "text-slate-900",
    },
    {
      label: "Equipment Used",
      value: data.kpis.equipmentUsed,
      icon: Truck,
      color: "text-slate-900",
    },
    {
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-slate-900">
            {data.project.name} - {data.site.name}
          </h1>
          <p className="mt-1 text-xs text-slate-500">
            Daily execution, labour, material, equipment, photos and issues.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
        <Panel title="Today's Site Report Checklist">
          ...
          <button className="mt-5 w-full rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-bold text-white">
        ...
        <Panel title="Site Photos" className="xl:col-span-2">
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            ...
            {!data.photos.length ? (
              <Empty text="No site photos uploaded." />
            ) : null}
          </div>
        </Panel>
      </div>
        ...
        <Panel title="Site Issues">
          <div className="space-y-3">
            {data.issues.map((issue) => (
              <div
                key={issue._id}
                className="flex items-center justify-between gap-3 rounded-md bg-orange-50 px-3 py-2 text-sm"
              >
