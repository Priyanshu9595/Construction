  ...
  const { data: materials, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/store/inventory?projectId=${projectId}`,
        {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-slate-500 mt-1">
            Track material stock levels, receipts, and consumption.
          </p>
        </div>
        <button
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-3 flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-slate-50 border-none rounded-lg pl-10 pr-4 py-2 outline-none focus:ring-2 focus:ring-blue-100"
          />
        </div>
      </div>
              ...
              {materials?.map((mat) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right">
                    ...
                    <span className="text-slate-500">{mat.unit || "nos"}</span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    {(mat.currentStock || 0) > 10 ? (
                      ...
                      <span className="px-2 py-1 bg-red-100 text-red-800 rounded-md text-xs font-bold uppercase">
                        Low Stock
                      </span>
                    )}
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-2xl max-w-sm w-full text-center">
            ...
            <p className="text-slate-500 mt-2">
              Material creation will be available shortly.
            </p>
            <button
              onClick={() => setIsModalOpen(false)}
              className="mt-4 px-4 py-2 bg-slate-200 rounded-lg font-bold"
            >
              Close
            </button>
          </div>
