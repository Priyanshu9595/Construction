  ...
  const { data: inspections, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/projects/${projectId}/operations/quality`,
        {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-slate-500 mt-1">
            Manage QA/QC checklists, inspections and NCRs.
          </p>
        </div>
        <button
              ...
              {inspections?.map((insp) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-extrabold text-slate-900">
                          {insp.inspectionType || "General Check"}
                        </p>
                        <p className="text-xs text-slate-500 mt-0.5">
                          Inspector: {insp.inspectorId?.name || "Admin"}
                        </p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 font-semibold text-slate-700">
                    {new Date(
                      insp.inspectionDate || insp.createdAt,
                    ).toLocaleDateString()}
                  </td>
                  <td className="px-6 py-4">
                    <span
      ...
      {isModalOpen && (
        <AddModal
          ...
          onSuccess={() => {
            ...
            queryClient.invalidateQueries({ queryKey: ["project-quality"] });
          }}
        />
      )}
    </div>
