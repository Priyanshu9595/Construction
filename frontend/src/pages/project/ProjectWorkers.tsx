...
import { Plus, Loader2, UserPlus, HardHat, Mail, Phone, IndianRupee, Wallet, History } from "lucide-react";
...
export default function ProjectWorkers() {
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedWorker && <PayWorkerModal projectId={projectId!} worker={selectedWorker} onClose={() => setSelectedWorker(null)} onSuccess={() => { setSelectedWorker(null); queryClient.invalidateQueries({ queryKey: ["project-workers"] }); }} />}
      {historyWorker && <WorkerPaymentHistoryModal projectId={projectId!} worker={historyWorker} onClose={() => setHistoryWorker(null)} />}
    </div>
  );
}
...
function PayWorkerModal({ projectId, worker, onClose, onSuccess }: { projectId: string, worker: any, onClose: () => void, onSuccess: () => void }) {
  ...
  useEffect(() => {
    async function fetchAttendance() {
      ...
      try {
        const token = localStorage.getItem("token");
        const res = await fetch(`${API_BASE_URL}/api/projects/${projectId}/workers/${worker._id}/attendance-summary?month=${formData.month}`, {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
        });
        if (res.ok) {
          ...
          if (data.alreadyPaid) {
            ...
            setFormData(prev => ({ ...prev, daysWorked: data.daysWorked || 0, grossSalary: 0, deductions: 0 }));
          } else {
    ...
    fetchAttendance();
  }, [formData.month, projectId, worker._id, worker.dailyWage]);

  const netSalary = Math.max(0, Number(formData.grossSalary) - Number(formData.deductions));

  const mutation = useMutation({
    mutationFn: async (data: any) => {
      const token = localStorage.getItem("token");
      // Format month back to a readable string for the DB if desired, but YYYY-MM is standard. Let's keep YYYY-MM.
      const res = await fetch(`${API_BASE_URL}/api/projects/${projectId}/workers/${worker._id}/pay`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
      <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(formData); }} className="w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50">
          ...
          <p className="text-sm font-semibold text-slate-500 mt-1">Generate a salary slip for {worker.firstName} {worker.lastName}.</p>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <label className="block text-sm font-bold text-slate-700">Salary Month / Period
              <input required type="month" value={formData.month} onChange={(e) => setFormData({...formData, month: e.target.value})} className="mt-1.5 w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 outline-none focus:border-blue-500" />
            </label>
            <label className="block text-sm font-bold text-slate-700">
              Days Worked {isLoadingAttendance && <Loader2 size={14} className="inline animate-spin ml-1 text-blue-500" />}
              <input disabled={alreadyPaid} required type="number" step="0.5" min="0" max="31" value={formData.daysWorked} onChange={(e) => handleDaysChange(Number(e.target.value))} className="mt-1.5 w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 outline-none focus:border-blue-500 disabled:opacity-50" />
            </label>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <label className="block text-sm font-bold text-slate-700">Gross Salary (₹)
              <input disabled={alreadyPaid} required type="number" value={formData.grossSalary} onChange={(e) => setFormData({...formData, grossSalary: Number(e.target.value)})} className="mt-1.5 w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 outline-none focus:border-blue-500 disabled:opacity-50" />
            </label>
            <label className="block text-sm font-bold text-slate-700">Deductions / Advances (₹)
              <input disabled={alreadyPaid} required type="number" value={formData.deductions} onChange={(e) => setFormData({...formData, deductions: Number(e.target.value)})} className="mt-1.5 w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 outline-none focus:border-blue-500 disabled:opacity-50" />
            </label>
          </div>
          
          {alreadyPaid ? (
            <div className="bg-amber-50 rounded-xl p-4 flex flex-col gap-1 border border-amber-200">
              <span className="font-bold text-amber-800">Payment Already Settled</span>
              <span className="text-sm font-medium text-amber-700">A payment of ₹{paidAmount} was already made for this month.</span>
              <div className="flex justify-between mt-2 pt-2 border-t border-amber-200/50">
                ...
                <span className="text-xl font-extrabold text-slate-400">₹0</span>
              </div>
            </div>
          ) : (
            <div className="bg-emerald-50 rounded-xl p-4 flex items-center justify-between border border-emerald-100">
              <span className="font-bold text-emerald-800">Net Payable Amount</span>
              <span className="text-xl font-extrabold text-emerald-700">₹{netSalary}</span>
            </div>
          )}

          {!alreadyPaid && <p className="text-xs font-semibold text-slate-500 text-center">Calculated at a daily wage of ₹{worker.dailyWage || 0}</p>}

          {mutation.isError && <div className="text-red-600 text-sm font-bold bg-red-50 p-3 rounded-lg border border-red-100">{mutation.error instanceof Error ? mutation.error.message : 'Error recording payment'}</div>}
        </div>
        <div className="px-6 py-5 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
          <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-xl font-bold text-slate-600 hover:bg-slate-200">Cancel</button>
          {!alreadyPaid && (
            <button type="submit" disabled={mutation.isPending} className="px-6 py-2.5 rounded-xl font-bold text-white bg-emerald-600 hover:bg-emerald-700 flex items-center gap-2">
              {mutation.isPending && <Loader2 size={16} className="animate-spin" />} Confirm Payment
            </button>
          )}
        </div>
      </form>
...
function WorkerPaymentHistoryModal({ projectId, worker, onClose }: { projectId: string, worker: any, onClose: () => void }) {
  const { data: payments, isLoading } = useQuery({
    queryKey: ["worker-payments", worker._id],
    queryFn: async () => {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_BASE_URL}/api/projects/${projectId}/workers/${worker._id}/payments`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      ...
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm font-semibold text-slate-500 mt-1">Past salary slips for {worker.firstName} {worker.lastName}.</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-full w-8 h-8 flex items-center justify-center font-bold">×</button>
        </div>
        ...
        <div className="p-0 overflow-y-auto">
          {isLoading ? (
            ...
            <div className="p-10 text-center text-slate-500 font-bold">No payment history found for this worker.</div>
          ) : (
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50/50 border-b border-slate-100 text-slate-500 text-xs uppercase tracking-wider font-extrabold">
                <tr>
              ...
              <tbody className="divide-y divide-slate-100">
                {payments?.map((payment: any) => (
                  <tr key={payment._id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 font-bold text-slate-800">{payment.month}</td>
                    <td className="px-6 py-4 font-medium text-slate-600">
                      {payment.paymentDate ? new Date(payment.paymentDate).toLocaleDateString() : 'N/A'}
                    </td>
                    <td className="px-6 py-4 text-slate-600">₹{payment.grossSalary}</td>
                    <td className="px-6 py-4 text-red-600">₹{payment.deductions}</td>
                    <td className="px-6 py-4 font-extrabold text-emerald-600">₹{payment.netSalary}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="px-6 py-5 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3 mt-auto">
          <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-xl font-bold text-slate-600 bg-slate-200 hover:bg-slate-300">Close</button>
        </div>
      </div>
    </div>
  );
