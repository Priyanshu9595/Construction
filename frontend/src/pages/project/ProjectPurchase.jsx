  ...
  const { data: pos, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/projects/${projectId}/operations/purchase-orders`,
        {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm font-medium text-slate-500 mt-1">
            Manage vendor POs, material requests and procurement.
          </p>
        </div>
        <button
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        {isLoading ? (
          <div className="p-10 flex items-center justify-center text-slate-500 font-bold">
            <Loader2 className="animate-spin mr-2" size={20} /> Loading purchase
            orders...
          </div>
        ) : pos?.length === 0 ? (
