...
export default function SimpleRoleDashboard({
  title,
  subtitle,
  endpoint,
  accent,
}) {
  const q = useQuery({ queryKey: [endpoint], queryFn: () => apiGet(endpoint) });
  if (q.isLoading) return <State text={`Loading ${title}...`} />;
  if (q.isError || !q.data)
    return (
      <State
        text={
          q.error instanceof Error ? q.error.message : `Unable to load ${title}`
        }
        error
      />
    );
    ...
    <div className="space-y-5">
      <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-xs text-slate-500">
          All cards and tables are loaded from protected MongoDB APIs.
        </p>
      </div>
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={normaliseChart(chartRows).slice(0, 5)}
                  dataKey="value"
                  innerRadius={58}
                  outerRadius={82}
                >
                  {normaliseChart(chartRows)
                    .slice(0, 5)
                    .map((r, i) => (
                      <Cell
                        key={r.name}
          ...
          {recentRows(data).map((row, index) => (
            ...
            >
              ...
              <span className="font-semibold text-slate-500">
                {row.status || row.result || row.amount || ""}
              </span>
            </div>
          ))}
          {!recentRows(data).length ? (
            <div className="rounded bg-slate-50 p-5 text-center text-sm font-bold text-slate-500">
              No records yet.
            </div>
          ) : null}
        </div>
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        {label.includes("Safety") || label.includes("Quality") ? (
          <ShieldCheck className={accent} size={18} />
        ) : label.includes("Amount") ||
          label.includes("Revenue") ||
          label.includes("Pay") ||
          label.includes("Income") ||
          label.includes("Expense") ? (
          <IndianRupee className={accent} size={18} />
        ) : (
          <ClipboardCheck className={accent} size={18} />
        )}
      </div>
